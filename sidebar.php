	...
	<?php if ( is_archive() || is_single() ) : ?>
		...
		<div class="bg-faded sidebar-nav">
			<div id="primary-two" class="widget-area">
				<?php
					$output = '<ul class="recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							$output .= '<li><h3>' . __( 'Recent Posts', 'startup' ) . '</h3></li>';
							while ( $recentposts_query->have_posts() ) :
								...
								$output .= '<h4><a href="' . get_the_permalink() . '" title="' . sprintf( __( 'Permalink to %s', 'startup' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' . get_the_title() . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
				...
				<br />
				<ul>
					<li><h3><?php _e( 'Categories', 'startup' ); ?></h3></li>
					<?php
					...
					<?php if ( ! is_author() ) : ?>
						<li>&nbsp;</li>
						<li><a href="<?php echo get_the_permalink( get_option( 'page_for_posts' ) ); ?>" class="btn btn-outline-secondary"><?php _e( 'more', 'startup' ); ?></a></li>
					<?php endif; ?>
				</ul>
			</div><!-- /#primary-two -->
		</div>
