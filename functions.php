...
$theme_version = '2.0';
	 ...
	 * @since v1.0
	 */
	if ( ! function_exists( 'startup_setup_theme' ) ) :
		function startup_setup_theme() {
		...
		add_action( 'after_setup_theme', 'startup_setup_theme' );
	endif;
	 ...
	 * @since v1.0
	 */
	if ( ! function_exists( 'startup_add_user_fields' ) ) :
		function startup_add_user_fields( $fields ) {
			// Add new fields
		...
		add_filter( 'user_contactmethods', 'startup_add_user_fields' ); // get_user_meta( $user->ID, 'facebook_profile', true );
	endif;
	 ...
	 * @since v1.0
	 */
	function startup_filter_media_comment_status( $open, $post_id ) {
	...
	add_filter( 'comments_open', 'startup_filter_media_comment_status', 10 , 2 );
	 ...
	 * @since v1.0
	 */
	function startup_custom_edit_post_link( $output ) {
		$output = str_replace( 'class="post-edit-link"', 'class="post-edit-link badge badge-info"', $output );
		return $output;
	}
	add_filter( 'edit_post_link', 'startup_custom_edit_post_link' );
	 ...
	 * @since v1.0
	 */
	function startup_oembed_filter( $html, $url, $attr, $post_id ) {
		$return = '<div class="embed-responsive embed-responsive-16by9">' . $html . '</div>';
		return $return;
	}
	add_filter( 'embed_oembed_html', 'startup_oembed_filter', 10, 4 );
		...
		function startup_content_nav( $nav_id ) {
			...
			if ( $wp_query->max_num_pages > 1 ) : ?>
				<div id="<?php echo $nav_id; ?>" class="d-flex mb-4 justify-content-between">
					<div><?php next_posts_link( '<span aria-hidden="true">&larr;</span> ' . __( 'Older posts', 'startup' ) ); ?></div>
					<div><?php previous_posts_link( __( 'Newer posts', 'startup' ) . ' <span aria-hidden="true">&rarr;</span>' ); ?></div>
				</div><!-- /.d-flex -->
			<?php
			else :
	...
	if ( ! function_exists( 'startup_article_posted_on' ) ) :
		...
		function startup_article_posted_on() {
			...
			printf( __( '<span class="sep">Posted on </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a><span class="by-author"> <span class="sep"> by </span> <span class="author-meta vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'startup' ),
	...
	function startup_password_form() {
		...
		$output = '<div class="row">';
			...
			$output .= '<h4 class="col-md-12 alert alert-warning">' . __( 'This content is password protected. To view it please enter your password below.', 'startup' ) . '</h4>';
				$output .= '<div class="col-md-6">';
					$output .= '<div class="input-group">';
						$output .= '<input type="password" name="post_password" id="' . $label . '" placeholder="' . __( 'Password', 'startup' ) . '" class="form-control" />';
						$output .= '<div class="input-group-append"><input type="submit" name="submit" class="btn btn-primary" value="' . esc_attr( __( 'Submit', 'startup' ) ) . '" /></div>';
	...
	add_filter( 'the_password_form', 'startup_password_form' );
	...
	if ( ! function_exists( 'startup_comment' ) ) :
		 ...
		 * add function to comments.php ... wp_list_comments( array( 'callback' => 'startup_comment' ) );
		 *
		 * @since v1.0
		 */
		function startup_comment( $comment, $args, $depth ) {
			...
			<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<footer class="comment-meta">
						<div class="comment-author vcard">
							<?php
								...
								printf( __( '%1$s, %2$s', 'startup' ),
									sprintf( '<span class="fn">%s</span>', get_comment_author_link() ),
									sprintf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
										esc_url( get_comment_link( $comment->comment_ID ) ),
										get_comment_time( 'c' ),
										/* translators: 1: date, 2: time */
										//sprintf( __( '%1$s - %2$s', 'startup' ), get_comment_time( $theme_dateformat ), get_comment_time( $theme_timeformat ) )
										sprintf( __( '%1$s ago', 'startup' ), human_time_diff( get_comment_time( 'U' ), current_time( 'timestamp' ) ) )
									)
								);
							?>
					...
					<div class="reply">
						<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'startup' ) . ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
					</div><!-- .reply -->
				</article><!-- #comment-## -->
		 ...
		 */
		function startup_custom_commentform( $args = array(), $post_id = null ) {
			if ( null === $post_id ) {
				$post_id = get_the_ID();
			}
			...
			$defaults = array(
				'fields'               => $fields,
				'comment_field'        => '<div class="form-group"><textarea id="comment" name="comment" class="form-control" aria-required="true" required placeholder="' . __( 'Comment', 'startup' ) . ( $req ? '*' : '' ) . '"></textarea></div>',
				/** This filter is documented in wp-includes/link-template.php */
				'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'startup' ), wp_login_url( apply_filters( 'the_permalink', get_the_permalink( get_the_ID() ) ) ) ) . '</p>',
				/** This filter is documented in wp-includes/link-template.php */
				'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'startup' ), get_edit_user_link(), $user->display_name, wp_logout_url( apply_filters( 'the_permalink', get_the_permalink( get_the_ID() ) ) ) ) . '</p>',
				'comment_notes_before' => '',
				'comment_notes_after'  => '<p class="small comment-notes">' . __( 'Your Email address will not be published.', 'startup' ) . '</p>',
				'id_form'              => 'commentform',
				'id_submit'            => 'submit',
				'class_submit'         => 'btn btn-primary',
				'name_submit'          => 'submit',
				'title_reply'          => '',
				'title_reply_to'       => __( 'Leave a Reply to %s', 'startup' ),
				'cancel_reply_link'    => __( 'Cancel reply', 'startup' ),
				'label_submit'         => __( 'Post Comment', 'startup' ),
		...
		add_filter( 'comment_form_defaults', 'startup_custom_commentform' );
	 ...
	 * @since v1.0
	 */
	function startup_scripts_loader() {
		...
		wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/vendor/font-awesome/css/font-awesome.min.css', false );
